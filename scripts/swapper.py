...
from scripts.globals import FACE_MODELS_PATH
...
from modules.mask import apply_face_mask
...
def clear_faces_list():
    ...
    SOURCE_FACES_LIST = []
    SOURCE_IMAGE_LIST_HASH = []
    logger.job("Source Images Hash has been reset (for Multiple or Folder Source)")
...
def restore_face(image: Image, enhancement_options: EnhancementOptions):
    ...
    if enhancement_options.face_restorer is not None:
        ...
        logger.job("Restoring the face with %s", enhancement_options.face_restorer.name())
...
def get_face_gender(
    ...
    if (gender_condition == 1 and face_gender == "F") or (gender_condition == 2 and face_gender == "M"):
        logger.job("OK - Detected Gender matches Condition")
        try:
            return sorted(face, key=lambda x: x.bbox[0])[face_index], 0
        except IndexError:
            return None, 0
    else:
        logger.job("WRONG - Detected Gender doesn't match Condition")
...
def half_det_size(det_size):
    logger.job("Trying to halve 'det_size' parameter")
    ...
    if model is not None:
        ...
        if (select_source == 0 and source_imgs is not None) or (select_source == 2 and (source_folder is not None and source_folder != "")):
            ...
            if random_image and select_source == 2:
                ...
                logger.job("Processing with Random Image from the folder")
            else:
                ...
                for i, source_image in enumerate(source_images):
                    ...
                    if source_hash_check:
                        ...
                        if len(SOURCE_FACES_LIST) == 0:
                            logger.job(f"Analyzing Source Image {i}...")
                            ...
                            logger.job("(Image %s) Using Hashed Source Face(s) Model...", i)
            ...
            if len(source_faces_ff) > 0:
                ...
                if target_hash_check:
                    ...
                    if TARGET_FACES is None or not target_image_same:
                        logger.job("Analyzing Target Image...")
                        target_faces = analyze_faces(target_img)
                        TARGET_FACES = target_faces
                    elif target_image_same:
                        logger.job("Using Hashed Target Face(s) Model...")
                        target_faces = TARGET_FACES
                ...
                for i,source_faces in enumerate(source_faces_ff):
                    ...
                    logger.job("(Image %s) Detecting Source Face, Index = %s", i, source_faces_index[0])
                    ...
                    if source_age != "None" or source_gender != "None":
                        logger.job("(Image %s) Detected: -%s- y.o. %s", i, source_age, source_gender)

                    if len(source_faces_index) != 0 and len(source_faces_index) != 1 and len(source_faces_index) != len(faces_index):
                        logger.job("Source Faces must have no entries (default=0), one entry, or same number of entries as target faces.")
        ...
        # SWAP from IMG or MODEL:
            ...
            if select_source == 0 and source_img is not None:
                ...
                if source_hash_check:
                    ...
                    if SOURCE_FACES is None or not source_image_same:
                        logger.job("Analyzing Source Image...")
                        source_faces = analyze_faces(source_img)
                        SOURCE_FACES = source_faces
                    elif source_image_same:
                        logger.job("Using Hashed Source Face(s) Model...")
                        source_faces = SOURCE_FACES
                ...
                if source_face_model is not None:
                    ...
                    logger.job("Using Loaded Source Face Model...")
                else:
            ...
            if source_faces is not None:
                ...
                logger.job("Detecting Source Face, Index = %s", source_faces_index[0])
                if select_source == 0 and source_img is not None:
                ...
                if source_age != "None" or source_gender != "None":
                    logger.job("Detected: -%s- y.o. %s", source_age, source_gender)
                ...
                if len(source_faces_index) != 0 and len(source_faces_index) != 1 and len(source_faces_index) != len(faces_index):
                    ...
                    logger.job("No source face(s) in the provided Index")
            else:
                logger.job("No source face(s) found")
...
def build_face_model(image: Image.Image, name: str):
    ...
    logger.job("Building Face Model...")
...
def operate(
    ...
    for face_num in faces_index:
        ...
        if len(source_faces_index) > 1 and source_face_idx > 0:
            logger.job("Detecting Source Face, Index = %s", source_faces_index[source_face_idx])
        ...
        if source_face is not None and wrong_gender == 0:
            logger.job("Detecting Target Face, Index = %s", face_num)
            ...
            if target_age != "None" or target_gender != "None":
                logger.job("Detected: -%s- y.o. %s", target_age, target_gender)
